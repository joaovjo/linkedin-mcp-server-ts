...
import { type ActionSignals, detectConnectionState, readActionSignals } from "./connection.ts";
...
async function openMoreMenu(): Promise<boolean> {
	return await browserManager.evaluate<boolean>(() => {
		...
		const expanders = [...main.querySelectorAll("button[aria-expanded]")] as HTMLButtonElement[];
...
async function dialogIsOpen(): Promise<boolean> {
	return await browserManager.evaluate<boolean>(`() => !!(document.querySelector('dialog[open], [role="dialog"]'))`);
}
...
async function submitInviteDialog(note?: string): Promise<{
	...
	if (note) {
		const hasTextarea = await browserManager.waitForSelector('[role="dialog"] textarea, dialog textarea', 1500);
		if (!hasTextarea) {
		...
		const boxReady = await browserManager.waitForSelector('[role="dialog"] textarea, dialog textarea', 3000);
		if (!boxReady) {
			const upsell = await getPremiumUpsellMessage();
			await dismissDialog();
			return {
...
export async function connectWithPerson(linkedinUsername: string, note?: string): Promise<Record<string, unknown>> {
	...
	if (!pageText) {
		return connectionResult(url, "unavailable", "Could not read profile page.", { profile: "" });
	}
	...
	if (state === "self_profile") {
		return connectionResult(url, "connect_unavailable", "Cannot send a connection request to your own profile.", {
			profile: pageText,
		});
	}
	if (state === "already_connected") {
		return connectionResult(url, "already_connected", "You are already connected with this profile.", { profile: pageText });
	}
	if (state === "pending") {
		return connectionResult(url, "pending", "A connection request is already pending for this profile.", { profile: pageText });
	}

	if (state === "incoming_request") {
		const clicked = await clickIncomingAccept();
		if (!clicked) {
			return connectionResult(url, "send_failed", "Could not find or click the Accept button.", { profile: pageText });
		}
		await Bun.sleep(3000);
		signals = await readActionSignals(linkedinUsername);
		state = detectConnectionState(signals);
		if (state !== "already_connected") {
			return connectionResult(url, "send_failed", "Accepted, but the profile did not transition to 1st-degree.", {
				profile: pageText,
			});
		}
	...
	if (!signals.has_invite_anchor) {
		...
		return connectionResult(url, "connect_unavailable", "LinkedIn did not expose a usable Connect action for this profile.", {
			profile: pageText,
		});
	}

	await browserManager.navigate(inviteUrl);
	const { submitted, note_sent, note_limit_message } = await submitInviteDialog(note);

	if (note_limit_message) {
		return connectionResult(url, "custom_note_limit_reached", note_limit_message, { note_sent: false, profile: pageText });
	}
	if (!submitted) {
		return connectionResult(url, "connect_unavailable", "LinkedIn did not open a usable invite dialog for this profile.", {
			profile: pageText,
		});
	}
	...
	if (after.has_invite_anchor) {
		return connectionResult(url, "send_failed", "Invite dialog submitted, but Connect action is still available.", {
			note_sent,
			profile: pageText,
		});
	}

	return connectionResult(url, "connected", note_sent ? "Connection request sent with note." : "Connection request sent.", {
		note_sent,
		profile: pageText,
	});
}
