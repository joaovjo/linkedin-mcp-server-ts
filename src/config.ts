...
export function loadConfig(args?: string[]): AppConfig {
  const parsed = parseArgs({
    ...
    options: {
      ...
      "login-timeout": { type: "string", default: env("LOGIN_TIMEOUT", "1800") },
      "log-level": { type: "string", default: env("LOG_LEVEL", "WARNING") },
      "profile-dir": { type: "string", default: env("PROFILE_DIR", "") },
      transport: { type: "string", default: env("TRANSPORT", "streamable-http") },
  ...
  return {
    port: parseInt(parsed.values["port"]!, 10),
    headless: parsed.values["headless"] !== "false",
    toolTimeout: parseInt(parsed.values["tool-timeout"]!, 10),
    loginTimeout: parseInt(parsed.values["login-timeout"]!, 10),
    logLevel: parsed.values["log-level"]!,
    profileDir,
    transport: parsed.values["transport"] as "stdio" | "streamable-http",
