...
export async function checkLogin(): Promise<boolean> {
	try {
		...
		const currentUrl = await browserManager.evaluate<string>(
...
export async function performLogin(profileDir: string): Promise<void> {
	...
	await browserManager.navigate("https://www.linkedin.com/login");
	...
	throw new Error("Login timed out. Run `bun run login` again to retry.");
}
